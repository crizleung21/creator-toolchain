#!/usr/bin/env python3
"""Verify the two isolated Copilot CLI sessions used by Behavior QA."""
...
PROFILES = (
    ...
    {"role": "evaluator", "model": "auto", "agent": None},
)


class ModelProbeError(RuntimeError):
    """Raised when either isolated Copilot session is unavailable."""
...
def select_available_profiles(
    *,
    ...
    """Probe two separate Auto sessions; model-level diversity is not assumed."""
    ...
    for profile in PROFILES:
        ...
        try:
            result = client(prompt=PROBE_PROMPT, model=model, agent=None, timeout=timeout)
    ...
    if set(results) != {"response", "evaluator"}:
        raise ModelProbeError("response or evaluator Auto session is unavailable", probes)
    return {
        ...
        "rubric_blind_response": True,
        "model_level_independence": "not-guaranteed-by-auto-selection",
        "local_evidence_validator": "scripts/evaluate_behavior_observations.py",
