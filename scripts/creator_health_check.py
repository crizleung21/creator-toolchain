...
def _signal(signal_id: str, level: str, weight: int, path: str, message: str) -> dict[str, Any]:
    return {"signal_id": signal_id, "level": level, "weight": weight, "path": path, "message": message}
...
def _project_ids(root: Path) -> set[str]:
    try:
        ...
        return set()
    return {item.get("project_id") for item in projects if isinstance(item, dict) and isinstance(item.get("project_id"), str)}
...
def _plan_signals(root: Path, calculated: datetime, stale_days: int) -> list[dict[str, Any]]:
    ...
    for project_path in sorted(plans_root.glob("*/project.json")):
        try:
            ...
            signals.append(_signal("PLAN_INVALID", "red", 50, project_path.relative_to(root).as_posix(), str(exc)))
            continue
        if not isinstance(project, dict):
            signals.append(_signal("PLAN_INVALID", "red", 50, project_path.relative_to(root).as_posix(), "project.json root is not an object"))
            continue
        ...
        if stage in {"ideating", "planned", "graduated"} and updated is not None:
            ...
            if age > stale_days:
                signals.append(_signal("STALE_PLAN", "amber", 10, project_path.relative_to(root).as_posix(), f"Plan has not been updated for {age} days."))
    return signals
...
def _execution_signals(root: Path, registered: set[str]) -> list[dict[str, Any]]:
    ...
    for execution_dir in sorted(path for path in execution_root.iterdir() if path.is_dir()):
        ...
        if not state_path.is_file():
            signals.append(_signal("ORPHAN_EXECUTION", "red", 50, execution_dir.relative_to(root).as_posix(), "Execution directory has no execution-state.json."))
            continue
        try:
            state = json.loads(state_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            signals.append(_signal("EXECUTION_STATE_INVALID", "red", 50, state_path.relative_to(root).as_posix(), str(exc)))
            continue
        ...
        if isinstance(project_id, str) and project_id not in registered:
            signals.append(_signal("UNREGISTERED_EXECUTION", "amber", 15, execution_dir.relative_to(root).as_posix(), f"Execution project {project_id} is not registered in .creator/projects.json."))
        ...
        if current in {"DONE", "DONE_WITH_CONCERNS"}:
            for key in ("reconciliation", "summary", "state_update_proposal"):
                ...
                if not isinstance(relative, str) or not (execution_dir / relative).is_file():
                    signals.append(_signal("MISSING_RECONCILIATION", "red", 50, state_path.relative_to(root).as_posix(), f"Terminal execution is missing {key}."))
    return signals
...
def _repository_signals(root: Path) -> list[dict[str, Any]]:
    ...
    if not behavior_path.is_file():
        signals.append(_signal("BEHAVIOR_STATUS_MISSING", "red", 50, "docs/qa/behavior-acceptance-status.json", "Behavior evidence freshness status is missing."))
    else:
        try:
            behavior = json.loads(behavior_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            signals.append(_signal("BEHAVIOR_STATUS_INVALID", "red", 50, behavior_path.relative_to(root).as_posix(), str(exc)))
        else:
            status = behavior.get("status") if isinstance(behavior, dict) else None
            if status == "STALE":
                signals.append(_signal("BEHAVIOR_EVIDENCE_STALE", "amber", 20, behavior_path.relative_to(root).as_posix(), str(behavior.get("reason") or "Behavior evidence is stale.")))
            elif status not in {"CURRENT", "PASS"}:
                signals.append(_signal("BEHAVIOR_STATUS_INVALID", "red", 50, behavior_path.relative_to(root).as_posix(), f"Unsupported behavior status: {status!r}."))
    for finding in check_skill_mirror(root / ".agents/skills", root / "plugin/creator-toolchain/skills", write=False):
        signals.append(_signal("MIRROR_MISMATCH", "red", 50, "plugin/creator-toolchain/skills", finding))
    report_path = root / "docs/qa/package-integrity-report.json"
    if not report_path.is_file():
        signals.append(_signal("PACKAGE_REPORT_MISSING", "red", 50, report_path.relative_to(root).as_posix(), "Package integrity report is missing."))
    else:
        for finding in check_integrity_report(root, root / "plugin/creator-toolchain", report_path):
            signals.append(_signal("PACKAGE_INTEGRITY_FAILURE", "red", 50, report_path.relative_to(root).as_posix(), finding))
    return signals


def calculate_health(root: Path, *, calculated_at: str | None = None, stale_plan_days: int = 30, include_repository_checks: bool = True, schema_root: Path = ROOT) -> dict[str, Any]:
    ...
    signals: list[dict[str, Any]] = []
    for finding in validate_workspace(root, schema_root=Path(schema_root).resolve()):
    ...
    signals = sorted(signals, key=lambda item: (LEVEL_ORDER[item["level"]], item["signal_id"], item["path"], item["message"]))
    counts = {"amber": sum(item["level"] == "amber" for item in signals), "red": sum(item["level"] == "red" for item in signals)}
    score = sum(int(item["weight"]) for item in signals)
    level = "red" if counts["red"] else "amber" if counts["amber"] else "green"
    next_action = "Resolve the first red health signal, then rerun the health check." if level == "red" else "Resolve the first amber health signal before release." if level == "amber" else "No corrective action is required."
    report = {
    ...
    findings = validate_json_schema(report, load_schema(Path(schema_root).resolve() / HEALTH_SCHEMA_RELATIVE))
    if findings:
        raise HealthCheckError("health report failed schema validation: " + "; ".join(findings))
    return report


def write_health(root: Path, report: dict[str, Any], *, schema_root: Path = ROOT) -> None:
    root = Path(root).resolve()
    ...
    state["state_divergence"] = {"score": report["score"], "level": report["level"], "notes": f"{report['summary']} Evidence: {HEALTH_REPORT_RELATIVE.as_posix()}."}
    ...
    try:
        ...
        if workspace_findings:
            raise HealthCheckError("post-write workspace validation failed: " + "; ".join(workspace_findings))
    except Exception:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = calculate_health(args.root, calculated_at=args.calculated_at, stale_plan_days=args.stale_plan_days)
        if args.write:
