...
def audit_document(document: dict[str, Any], *, audited_at: str | None = None, schema_root: Path = ROOT) -> dict[str, Any]:
    ...
    for rule_id, items in sorted(by_rule_id.items()):
        if rule_id and len(items) > 1:
            conflicts.append(_conflict("duplicate", "critical", f"Rule ID {rule_id} appears more than once.", domains=[item[0] for item in items], rule_ids=[rule_id], evidence=[str(item[1].get("text", "")) for item in items]))
    for normalized, items in sorted(by_text.items()):
        ids = [str(item[1].get("rule_id", "")) for item in items]
        if normalized and len(set(ids)) > 1:
            conflicts.append(_conflict("duplicate", "high", "Active rules contain duplicate normalized text.", domains=[item[0] for item in items], rule_ids=ids, evidence=[str(item[1].get("text", "")) for item in items]))
    ...
    for base, items in sorted(polarity_groups.items()):
        if len({item[2] for item in items}) > 1:
            conflicts.append(_conflict("contradiction", "critical", f"Rules express opposite polarity for: {base}.", domains=[item[0] for item in items], rule_ids=[str(item[1].get("rule_id", "")) for item in items], evidence=[str(item[1].get("text", "")) for item in items]))
    ...
    for index, first in enumerate(enabled):
        for second in enabled[index + 1:]:
            ...
            if "GLOBAL" in {first_id, second_id}:
                continue
            shared = sorted(set(first.get("trigger_keywords", [])) & set(second.get("trigger_keywords", [])))
            same_scope = str(first.get("scope", "")).casefold() == str(second.get("scope", "")).casefold()
            if shared or same_scope:
                conflicts.append(_conflict("scope_overlap", "medium", f"Domains {first_id} and {second_id} overlap in scope or triggers.", domains=[first_id, second_id], evidence=shared or [str(first.get("scope", ""))]))

    unsafe_terms = ("bypass approval", "silently mutate", "delete without", "disable validation", "ignore safety", "skip verification")
    for domain_id, rule in rules:
        if rule.get("status", "active") != "active":
            continue
        rule_id = str(rule.get("rule_id", ""))
        ...
        if review_date is not None and review_date < now:
            conflicts.append(_conflict("stale_rule", "medium", f"Rule {rule_id} passed its review date.", domains=[domain_id], rule_ids=[rule_id], evidence=[str(rule.get("review_date"))]))
        ...
        if domain_id != "GLOBAL" and (scope in {"*", "all", "everything", "any task"} or " always " in _normalize(text)):
            conflicts.append(_conflict("overbroad_rule", "medium", f"Rule {rule_id} may be broader than its domain.", domains=[domain_id], rule_ids=[rule_id], evidence=[text]))
        lowered = text.casefold()
        matched = []
        for term in unsafe_terms:
            if term not in lowered:
                continue
            prohibited_forms = (f"do not {term}", f"must not {term}", f"never {term}", f"cannot {term}")
            if any(form in lowered for form in prohibited_forms):
                continue
            matched.append(term)
        if matched:
            conflicts.append(_conflict("unsafe_rule", "critical", f"Rule {rule_id} contains an unsafe governance instruction.", domains=[domain_id], rule_ids=[rule_id], evidence=matched))
    ...
    for command_id, items in sorted(by_command_id.items()):
        if command_id and len(items) > 1:
            conflicts.append(_conflict("duplicate_command", "critical", f"Command ID {command_id} appears more than once.", domains=[item[0] for item in items], command_ids=[command_id]))
    for key, items in sorted(by_command_key.items()):
        ids = [str(item[1].get("command_id", "")) for item in items]
        if key[0] and len(set(ids)) > 1:
            conflicts.append(_conflict("duplicate_command", "high", "Active commands share the same trigger and workflow.", domains=[item[0] for item in items], command_ids=ids, evidence=[f"trigger={key[0].strip()}", f"workflow={key[1]}"]))
    ...
    for decision in document.get("decision_log", []):
        ...
        if review_date is not None and review_date < now:
            ...
            conflicts.append(_conflict("stale_decision", "low", f"Decision {decision_id} passed its review date.", decision_refs=[decision_id], evidence=[str(decision.get("review_date"))]))

    unique = {item["conflict_id"]: item for item in conflicts}
    ordered = sorted(unique.values(), key=lambda item: (not item["blocking"], item["conflict_type"], item["conflict_id"]))
    report = {"schema_version": "1.0.0", "audited_at": audited_at, "status": "BLOCKED" if any(item["blocking"] for item in ordered) else "PASS_WITH_ADVISORIES" if ordered else "PASS", "blocking_count": sum(bool(item["blocking"]) for item in ordered), "advisory_count": sum(not bool(item["blocking"]) for item in ordered), "conflicts": ordered}
    findings = validate_json_schema(report, load_schema(Path(schema_root) / CONFLICT_REPORT_SCHEMA))
    if findings:
        raise RuleConflictError("conflict report failed schema validation: " + "; ".join(findings))
    return report


def audit_rules(root: Path, *, audited_at: str | None = None, schema_root: Path = ROOT) -> dict[str, Any]:
