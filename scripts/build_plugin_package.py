...
def build_plugin_archive(root: Path, package_root: Path, output: Path) -> dict[str, object]:
    """Compatibility API used by the unified release orchestrator.

    The current package contract permits exactly one package root. Requiring that
    path here prevents callers from silently building a different payload.
    """

    root = Path(root).resolve()
    expected = root / "plugin/creator-toolchain"
    if Path(package_root).resolve() != expected:
        raise ValueError(f"package_root must be {expected}")
    return build_plugin_package(root, output)
