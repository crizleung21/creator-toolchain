...
class ValidatorTests(unittest.TestCase):
    ...
    def _plugin_root(self) -> Path:
        ...
        manifest = {
            "name": "creator-toolchain", "version": read_version(ROOT),
            "description": "Creator Toolchain", "author": {"name": "crizleung21"},
            "license": "MIT", "skills": "./skills/",
            "interface": {"developerName": "crizleung21"},
        }
        (package / ".codex-plugin/plugin.json").write_text(json.dumps(manifest) + "\n", encoding="utf-8")
        marketplace = {"name": "creator-toolchain", "plugins": [{"name": "creator-toolchain", "source": {"source": "local", "path": "./plugin/creator-toolchain"}, "policy": {"installation": "AVAILABLE", "authentication": "ON_INSTALL"}, "category": "Productivity"}]}
        (root / ".agents/plugins/marketplace.json").write_text(json.dumps(marketplace) + "\n", encoding="utf-8")
        ...
        report_path.write_text(json.dumps(build_integrity_report(root, package), indent=2) + "\n", encoding="utf-8")
        return root
    ...
    def test_scope_plugin_passes_current_contract(self) -> None:
        self.assertEqual(validator.validate_plugin_package(self._plugin_root()), [])
    ...
    def test_scope_plugin_rejects_unexpected_package_file(self) -> None:
        ...
        self.assertIn("PACKAGE_INTEGRITY", self._ids(validator.validate_plugin_package(root)))

    def test_scope_plugin_rejects_stale_integrity_report(self) -> None:
        root = self._plugin_root()
        (root / "plugin/creator-toolchain/README.md").write_text("changed\n", encoding="utf-8")
        self.assertIn("PACKAGE_INTEGRITY_REPORT", self._ids(validator.validate_plugin_package(root)))

    def test_scope_plugin_requires_authoritative_version(self) -> None:
        root = self._plugin_root()
        manifest_path = root / "plugin/creator-toolchain/.codex-plugin/plugin.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["version"] = "1.0.0"
        manifest_path.write_text(json.dumps(manifest) + "\n", encoding="utf-8")
        self.assertIn("MANIFEST_VERSION", self._ids(validator.validate_plugin_package(root)))

    def test_scope_plugin_requires_license_parity(self) -> None:
        root = self._plugin_root()
        (root / "plugin/creator-toolchain/LICENSE").write_text("different\n", encoding="utf-8")
        self.assertIn("LEGAL_PARITY", self._ids(validator.validate_plugin_package(root)))
